...
- **Zero runtime risk** — Pure Markdown skill. This skill is guidance only and does not run code by itself.

## Installation

This skill ships in this repo's curated Codex skill bundle. No separate installation is needed when `skills/codex-team-skills/blueprint/SKILL.md` is present.
...
```bash
test -f skills/codex-team-skills/blueprint/SKILL.md
```

To update from upstream later, review the upstream diff before replacing the vendored copy:

```bash
cd /path/to/upstream/everything-claude-code
...
- Codex or another coding-agent harness that can read this skill
